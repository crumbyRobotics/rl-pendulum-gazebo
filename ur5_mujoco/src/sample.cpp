...
void mouse_button(GLFWwindow* window, int button, int act, int mods)
{
    ...
    if( act==GLFW_PRESS && glfwGetTime()-lastclicktm<0.25 && button==lastbutton )
    {
        ...
        int selgeom = mjv_select(m, d, &vopt,
                                 ...
                                 &scn, selpnt, m->geom_bodyid, m->skin_matid);
...
void init(void)
{
    ...
    d->qpos[14] = -0.62;
    d->qpos[15] = 0.32;
    d->qpos[17] = cos(0.16 * pi);
    d->qpos[20] = sin(0.16 * pi);

    d->qpos[21] = -0.72;
    d->qpos[22] = 0.38;
    d->qpos[24] = cos(0.18 * pi);
    d->qpos[27] = sin(0.18 * pi);

    d->qpos[28] = -0.835;
    d->qpos[29] = 0.425;
    d->qpos[31] = cos(0.195 * pi);
    d->qpos[34] = sin(0.195 * pi);

    d->qpos[35] = -0.935;
    d->qpos[36] = 0.46;
    d->qpos[38] = cos(0.23 * pi);
    d->qpos[41] = sin(0.23 * pi);
}
